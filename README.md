...
- **Agent Skills** - Claude Code (`.claude/skills/`) / Codex (`.agents/skills/`) 両対応のスキルファイルを自動生成。エージェントが `ma-task` CLIでタスク管理可能
- **ma-task CLI** - エージェント向けタスク管理CLI。ステータス更新・リンク追加・タスク作成等をJSON出力で提供
